...
public class Lab16Main {
	...
	public Map<Integer, ArrayList<Movie>> byYearMap = new HashMap<>();
	public Map<String, ArrayList<Movie>> byGenreMap = new HashMap<>();
    ...
    */
	...
	public void readMovies(String filename) {
        ...
        */
		while (fileInput.hasNext()) {
			...
			// By year
			byNameMap.put(newMovie.getMovieName(), newMovie);

			// Add by Year
			ArrayList<Movie> aux;
			if ( byYearMap.get(newMovie.getYear()) != null){
				aux = byYearMap.get(newMovie.getYear());
			} else {
				aux = new ArrayList<>();
			}
			aux.add(newMovie);
			byYearMap.put(newMovie.getYear(), aux);

			// Add by Genre
			for (String genre: newMovie.getGenres()) {
				if ( byGenreMap.get(genre) != null){
					aux = byGenreMap.get(genre);
				} else {
					aux = new ArrayList<>();
				}
				aux.add(newMovie);
				byGenreMap.put(genre, aux);
			}
	...
	public ArrayList<Movie> searchByName(String name) {
		...
		// 	}
		// }
		if (byNameMap.get(name) != null) {
			list.add(byNameMap.get(name));
		}
